...
namespace Blog.Controllers
{
    ...
    public class ContentController : Controller
    {
        ...
        private readonly IWebHostEnvironment _appEnvironment;   // WebhostEnvirnment через него можно будет получать путь к корневой папки? 18-40 пригодится для ...там чтот опаппки изоброжениями чтоли?
        private readonly IPublicationEmailNotification _publicationEmailNotification;

        public ContentController(ICategory categories, IPublication publications, IWebHostEnvironment appEnvironment, IPublicationEmailNotification publicationEmailNotification)
        {
            _categories = categories;
            _publications = publications;
            _appEnvironment = appEnvironment;
            _publicationEmailNotification = publicationEmailNotification;
        }
        ...
        public async Task<IActionResult> CreatePublication(PublicationViewModel publicationViewModel, string[] categories)
        {
            if (ModelState.IsValid)
            {
                ...
                //hw уведомление подписчиков о публикации автора чрез имейл рассылку
                await _publicationEmailNotification.NotifySubscribersAsync(
                    publication.UserId,
                    publication.Title,
