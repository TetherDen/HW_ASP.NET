...
namespace Blog.Data
{
    public class ContentInitializer
    {
        public static async Task InitializeAsync(ApplicationContext context)
        {
            ...
            {
                // HW после того как сделал подписчиков надо добавить авторов публикаям...
                var alex = await context.Users.FirstOrDefaultAsync(u => u.Email == "alex@gmail.com");
                var marry = await context.Users.FirstOrDefaultAsync(u => u.Email == "marry@in.ua");
                if (alex != null && marry != null)
                {
